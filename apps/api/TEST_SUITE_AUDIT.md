...
`tests/integration/agents/test_vision_agent.py` (docstring: *"zero-mock, real-world integration test suite … real Gemini LLM"*) calls `_run_llm_member_extraction(...)` and `parser_node(...)`, which hit the live Gemini API and cost money / require network.
...
Meanwhile, real sample files *do* exist in-repo under `uploads/` (e.g. `uploads/PRJ-BCD1A535/..._Floor-beam.dxf` / `.pdf`), so the parsing portion could be exercised offline against a committed fixture.
...
The only var CI provides is `PROJECT_STORE_BACKEND`. Locally, the minimum needed to *collect* today is `PROJECT_STORE_BACKEND=memory GEMINI_API_KEY=dummy GOOGLE_API_KEY=dummy`.
