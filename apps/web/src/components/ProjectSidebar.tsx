...
function SidebarTooltip({ label, children }: SidebarTooltipProps) {
  return (
    <div className="relative group/tooltip">
      {children}
      <div className="absolute left-full ml-2.5 top-1/2 -translate-y-1/2 z-[60] hidden group-hover/tooltip:block pointer-events-none">
        <div className="bg-popover border border-border text-popover-foreground text-xs px-2 py-1 rounded-md whitespace-nowrap shadow-lg font-sans">
          {label}
        </div>
      </div>
    </div>
...
export function ProjectSidebar() {
  ...
  return (
    <>
      <aside
        className={cn(
          "h-full flex flex-col bg-card border-r border-border flex-shrink-0 overflow-hidden",
      ...
      >
          ...
          {sidebarExpanded ? (
            <>
              <div className="flex items-center gap-2 min-w-0">
                <Hexagon className="h-5 w-5 text-primary flex-shrink-0" />
                <span className="text-sm font-semibold tracking-tight whitespace-nowrap">StructAI</span>
                <span className="text-xs text-muted-foreground font-mono whitespace-nowrap">Copilot</span>
              </div>
              <button
        ...
        <div
          className={cn(
            "py-2 border-b border-border flex-shrink-0",
        ...
        <div className="flex-1 overflow-y-auto scrollbar-thin py-2 min-h-0">
          ...
          <ul className={cn("space-y-px", sidebarExpanded ? "px-2" : "px-1.5")}>
                  ...
                  {sidebarExpanded ? (
                    ...
                    >
                      {isOpening ? (
                        ...
                        <Folder className="h-4 w-4 flex-shrink-0 group-hover:text-primary transition-colors" />
                      )}
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium truncate leading-tight">{p.name}</p>
                        <p className="text-[10px] font-mono text-muted-foreground truncate">{p.reference}</p>
                      </div>
                      <span
                        className={cn(
                          "text-[10px] font-mono capitalize flex-shrink-0 hidden group-hover:hidden",
        ...
        <div
          className={cn(
            "py-2 border-t border-border flex-shrink-0",
        ...
        <div
          className={cn(
            "py-2 border-t border-border flex-shrink-0",
            sidebarExpanded ? "px-2 space-y-0.5" : "flex flex-col items-center gap-0.5"
          )}
        >
          {sidebarExpanded ? (
            <>
